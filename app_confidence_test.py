...
st.markdown("Upload an image to detect license plates and extract text using computer vision")
...
if uploaded_file is None:
    ...
    This version uses only OpenCV techniques:
    ...
    if st.button("🔍 Detect License Plates", type="primary"):
        with st.spinner("🔄 Processing image with OpenCV..."):
            ...
            if filtered_plates:
                overall_confidence = sum(p['confidence'] for p in filtered_plates) / len(filtered_plates)
            else:
                ...
                for i, plate in enumerate(filtered_plates):
                    ...
                    <div class="metric-container">
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <h3 style="margin: 0; color: white;">{confidence_emoji} License Plate {i+1}</h3>
